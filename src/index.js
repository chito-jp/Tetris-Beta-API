...
export class Beta {
  constructor(userAgent) {
    ...
    this.ws = new Mist({
      projectId: "1000009994",
      userAgent: userAgent || "chito-bot",
    });
    this.ws.on("set", (n, v) => {
      ...
      if (499 < res.code && res.code < 600) {
        ...
        for (const [req_id, req_code] of this.req.code.entries()) {
          if (
            req_code.startsWith(
              `${res.code - 400}${res.id}${res.UNN_length + 10}${res.UNN}`
            )
          ) {
        ...
        for (const [req_id, req_code] of this.req.code.entries()) {
          if (
            req_code.startsWith(
              `${res.code - 400}${res.playerId}${res.address}${res.id}`
            )
          ) {
            const resolve = this.req.resolve.get(req_id);
            if (resolve) resolve(res.data);
  ...
  toStr(num) {
    ...
    for (let i = 0; i < num.length / 2; i++)
      ret += char[Number(num[i * 2] + num[i * 2 + 1]) - 11];
    return ret;
  }
  getData() {
    return this.data;
  ...
  async getRanking(type, name, session) {
    ...
    return { ranking: this.sortRanking(raw, type), raw };
  }
  timeFormat(time) {
    let ret = "";
    let _ = String(time);
  ...
  timeFormat2(time) {
    ...
    for (let i = 0; i < 3; i++)
      if (i < 1) {
        ...
        if (String(times[i]).length < 2) ret += `0${times[i]}${":."[i] || ""}`;
        else ret += `${times[i]}${":."[i] || ""}`;
      }
    return ret;
  }
  ...
  config(raw, index) {
    return {
      ...
      keyconfig: {
        ...
        Rotation_Left:
          this.keys[Number(raw.slice(index + 128, index + 130)) - 11],
        Rotation_Right:
          this.keys[Number(raw.slice(index + 130, index + 132)) - 11],
  ...
  user(raw, index, option) {
    ...
    if (play_time / 60 >= 10) {
      play_time = Math.floor(play_time / 60);
    } else {
      play_time = Math.floor((play_time / 60) * 10) / 10;
    }
    return {
      name: option?.name,
      id: option?.id,
      ...
      "40line": {
        ...
        time: this.timeFormat(
          Number(raw.slice(index + 270, index + 279)) - 100000000
        ),
      ...
      "20line": {
        ...
        time: this.timeFormat(
          Number(raw.slice(index + 304, index + 313)) - 100000000
        ),
  ...
  async getUser(target, name, session) {
    const id = await this.getId(name);
    const req_code = `254${id}999${Math.floor(Math.random() * 9)}${
      session || 10000
    }${this.toNum(target)}`;
    const raw = await this.sendRequest(req_code);
    return { id, data: this.user(raw, 0, { name, id }), raw };
  }
}
